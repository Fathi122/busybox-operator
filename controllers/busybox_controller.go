...
import (
	...
	cachev1alpha1 "github.com/Fathi122/busybox-operator/api/v1alpha1"
)
...
//+kubebuilder:rbac:groups=cache.taf122,resources=busyboxes,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=cache.taf122,resources=busyboxes/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=cache.taf122,resources=busyboxes/finalizers,verbs=update
...
func (r *BusyboxReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Update the Busybox status with the pod names
	// List the pods for this busybox's deployment
	podList := &corev1.PodList{}
	listOpts := []client.ListOption{
		client.InNamespace(busybox.Namespace),
		client.MatchingLabels(labelsForBusybox(busybox.Name)),
	}
	if err = r.List(ctx, podList, listOpts...); err != nil {
		log.Error(err, "Failed to list pods", "Busybox.Namespace", busybox.Namespace, "busybox.Name", busybox.Name)
		return ctrl.Result{}, err
	}
	podNames := getPodNames(podList.Items)

	// Update status.Nodes if needed
	if !reflect.DeepEqual(podNames, busybox.Status.Nodes) {
		busybox.Status.Nodes = podNames
...
func (r *BusyboxReconciler) deploymentForBusybox(m *cachev1alpha1.Busybox) *appsv1.Deployment {
...
func labelsForBusybox(name string) map[string]string {
	return map[string]string{"app": "busybox", "busybox_cr": name}
}
